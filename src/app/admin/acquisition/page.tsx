...
import type { AcquisitionLead, LeadStatus } from "@/lib/admin/acquisition-mock-data";
  ...
  return (
    <div className="flex items-center justify-between px-5 flex-shrink-0" style={{ height: 58, background: T.bgSidebar, borderBottom: `1px solid ${T.border}` }}>
      <div className="flex items-center gap-2">
        ...
        <span style={{ fontFamily: "var(--font-heading)", fontWeight: 800, fontSize: 17, color: T.textPrimary }}>Acquisition</span>
      </div>
      <div className="flex items-center gap-2">
        <button
        ...
        <button
          ...
          onClick={() => console.log("add manual lead")}
        >+ Add lead</button>
      </div>
    </div>
  );
...
function ScraperHealthStrip() {
  ...
  const cards = [
    { label: "SCRAPER STATUS", value: "Running", valueColor: T.teal200, sub: `Last run ${S.lastRunMinutesAgo} min ago`, dot: T.green },
    { label: "LISTINGS FOUND TODAY", value: S.listingsFoundToday.toLocaleString(), valueColor: T.textPrimary, sub: S.listingsDelta, dot: T.green },
    { label: "OUTREACH SENT TODAY", value: S.outreachSentToday.toString(), valueColor: T.textPrimary, sub: `${S.outreachQueued} queued`, dot: T.green },
    { label: "NEXT SCRAPER RUN", value: `${S.nextRunMinutes} min`, valueColor: T.amber, sub: `Targeting ${S.nextRunTargets} make/models`, dot: "#FCD34D" },
  ];
...
function KpiStrip() {
  ...
  const cards = [
    { label: "ACTIVE LEADS", value: K.activeLeads.toString(), valueColor: T.textPrimary, delta: "In pipeline today", deltaColor: T.textMuted },
    { label: "RESPONDED", value: K.responded.toString(), valueColor: T.green, delta: `${K.responseRate} response rate`, deltaColor: T.green },
    { label: "NEGOTIATING", value: K.negotiating.toString(), valueColor: T.amber, delta: `AI handling ${K.aiHandling} · ${K.escalated} escalated`, deltaColor: T.textMuted },
    { label: "ACCEPTED TODAY", value: K.acceptedToday.toString(), valueColor: T.teal200, delta: `↑ ${K.acceptedDelta} vs yesterday`, deltaColor: T.green },
    { label: "ESCALATIONS OPEN", value: K.escalated.toString(), valueColor: T.red, delta: "Need human now", deltaColor: T.red },
  ];
...
function aiActionColor(status: LeadStatus): string {
...
function LeadTable({
  ...
  // Filter
  let filtered = ACQUISITION_LEADS;
  ...
  return (
    <div className="rounded-[10px] overflow-hidden min-w-0" style={{ background: T.bgCard, border: `1px solid ${T.border}` }}>
      ...
      <div className="flex items-center px-[14px] py-[10px]" style={{ borderBottom: `1px solid ${T.border}` }}>
        <span style={{ fontFamily: "var(--font-body)", fontWeight: 700, fontSize: 13, color: T.textPrimary }}>Lead pipeline</span>
        <span className="ml-auto" style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: 11, color: T.textMuted }}>{sorted.length} leads</span>
      </div>
      ...
      <div className="flex items-center gap-2 px-[14px] py-[6px]">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search leads, VIN, make/model…"
            value={searchQuery}
        ...
        <table className="w-full" style={{ minWidth: 700, tableLayout: "fixed" }}>
          ...
          <thead>
            <tr>
              ...
              {["Vehicle", "Seller", "Asking", "Offer", "Status", "AI action", "Age"].map((h) => (
                <th
                  key={h}
          ...
          <tbody>
            {sorted.map((lead, idx) => (
              <tr
                ...
                onClick={() => { console.log("navigate", lead.id); router.push(`/admin/acquisition/${lead.id}`); }}
              >
                ...
                <td className="px-[10px] py-2 align-middle" style={{ borderBottom: idx < sorted.length - 1 ? `1px solid ${T.border2}` : "none" }}>
                  <div style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: 12, color: T.textPrimary }}>{lead.year} {lead.make} {lead.model}</div>
        ...
        {ESCALATIONS.map((esc, idx) => {
          ...
          return (
            <div
              ...
              onClick={() => { console.log("escalation", esc.id); router.push(`/admin/acquisition/${esc.id}`); }}
  ...
  return (
    <div className="flex min-h-screen" style={{ background: T.bgPage }}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <Topbar selectedCount={selectedLeads.length} />
